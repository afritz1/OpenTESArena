...
class Job {
  public:
    ...
    Job(std::function<void()> &&task) : task(std::move(task)) {}
...
class Worker {
  private:
    ...
    std::condition_variable
        *pool__idle_notifier; // Ping this to tell the pool we're idle.
    std::atomic<std::uint32_t>
        *pool__idle_counter; // To allow the pool to easily check if it has idle
                             // workers.
  ...
  public:
    ...
    Worker(std::condition_variable *idle_notifier,
           std::atomic<std::uint32_t> *idle_counter)
    ...
    void invoke(std::function<void()> &&what)
    {
        ...
        this->context = std::thread(([this, what]() {
            what();
            this->notifyIdle(); // It's likely the pool is waiting for an idle
                                // worker.
        }));
    }
...
class ThreadPool {
  public:
    ...
    std::uint32_t idleWorkers() { return this->idle_workers.load(); }
...
class JobManager {
  public:
    ...
    bool isRunning() { return this->running.load(); }
