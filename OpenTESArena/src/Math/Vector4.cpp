...
Vector4<T>::Vector4(const Vector3<T> &v)
	: Vector4(v.getX(), v.getY(), v.getZ(), T()) { }
...
template<typename T>
Vector4<T> Vector4<T>::fromARGB(unsigned int argb)
{
	return Vector4(
...
template<typename T>
Vector4<T> Vector4<T>::fromColor(const Color &c)
{
	return Vector4(
...
template<typename T>
unsigned int Vector4<T>::toARGB() const
{
