	...
	if (voxelTraitsDef.type == ArenaTypes::VoxelType::Wall)
	{
			...
			&dirToWorldVoxel](int levelIndex)
		{
			...
			newActiveLevel.update(dummyDeltaTime, activeChunkPositions, newChunkPositions, freedChunkPositions,
				player.getPosition(), levelIndex, interiorMapDef, interiorMapInst, entityGenInfo, citizenGenInfo,
				gameState.getChasmAnimPercent(), game.getRandom(), game.getEntityDefinitionLibrary(),
				game.getBinaryAssetLibrary(), renderChunkManager, textureManager, game.getAudioManager(), renderer);
		};
