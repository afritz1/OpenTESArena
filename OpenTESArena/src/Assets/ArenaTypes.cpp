...
void ArenaTypes::Rect16::init(Span<const std::byte> exeBytes, int exeAddress)
{
	DebugAssert(exeBytes.isValidRange(exeAddress, ArenaTypes::Rect16::SIZE));
...
void ArenaTypes::List::init(Span<const std::byte> exeBytes, int exeAddress)
{
	DebugAssert(exeBytes.isValidRange(exeAddress, ArenaTypes::List::SIZE));
	this->buttonUp.init(exeBytes, exeAddress);
	this->buttonDown.init(exeBytes, exeAddress + ArenaTypes::Rect16::SIZE);
	this->scrollBar.init(exeBytes, exeAddress + (ArenaTypes::Rect16::SIZE * 2));
	this->area.init(exeBytes, exeAddress + (ArenaTypes::Rect16::SIZE * 3));

	const uint8_t *ptr = reinterpret_cast<const uint8_t*>(exeBytes.begin()) + exeAddress;
	this->flags = Bytes::getLE16(ptr + (ArenaTypes::Rect16::SIZE * 4));
}
...
ArenaBuildingType ArenaTypes::interiorTypeToBuildingType(ArenaInteriorType interiorType)
{
