...
namespace swGeometry
{
	...
	void ProcessMeshesForRasterizationInternal(int meshCount, const Matrix4d &viewMatrix, const Matrix4d &projectionMatrix)
	{
		for (int meshIndex = 0; meshIndex < meshCount; meshIndex++)
		{
			...
			for (int triangleIndex = 0; triangleIndex < triangleCount; triangleIndex++)
			{
				...
				meshProcessCache.clipSpaceTriangleV0s[0] = shadedV0;
				meshProcessCache.clipSpaceTriangleV1s[0] = shadedV1;
				meshProcessCache.clipSpaceTriangleV2s[0] = shadedV2;
				meshProcessCache.clipSpaceTriangleUV0s[0] = uv0;
				meshProcessCache.clipSpaceTriangleUV1s[0] = uv1;
				meshProcessCache.clipSpaceTriangleUV2s[0] = uv2;
	...
	void ProcessMeshesForRasterization(int meshCount, const Matrix4d &viewMatrix, const Matrix4d &projectionMatrix, VertexShaderType vertexShaderType)
	{
		// Dispatch based on vertex shader.
		switch (vertexShaderType)
		{
		case VertexShaderType::Basic:
			ProcessMeshesForRasterizationInternal<VertexShaderType::Basic>(meshCount, viewMatrix, projectionMatrix);
			break;
		case VertexShaderType::RaisingDoor:
			ProcessMeshesForRasterizationInternal<VertexShaderType::RaisingDoor>(meshCount, viewMatrix, projectionMatrix);
			break;
		case VertexShaderType::Entity:
			ProcessMeshesForRasterizationInternal<VertexShaderType::Entity>(meshCount, viewMatrix, projectionMatrix);
			break;
		default:
	...
	const RenderFrameSettings &settings, uint32_t *outputBuffer)
{
	...
	while (drawCallIndex < drawCallCount)
	{
		...
		// Process the draw calls' meshes and keep them in a global cache for rasterizing.
		swGeometry::ProcessMeshesForRasterization(drawCallSequenceCount, viewMatrix, projectionMatrix, vertexShaderType);
