...
Get the latest `data` and `options` folders [here](https://www.dropbox.com/s/xc8llh52eahaofs/OpenTESArena_data.zip?dl=0) (last updated September 5th). The program's dependency on the `data` folder will mostly be removed once the original assets are being loaded exclusively.
